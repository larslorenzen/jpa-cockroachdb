...
public class MessageSender {
    ...
    public void transactionChanged(@Observes TransactionChangedEvent event) {
        ...
        TransactionChangedMessage message = new TransactionChangedMessage(transaction.getId() + "", transaction.getAccount().getPlayerReference(), transaction.getReference(),
        ...
        try (KafkaConnection connection = factory.createConnection()) {
            connection.send(new ProducerRecord<>("accountUpdates", transaction.getAccount().getPlayerReference(), toJsonString(message)));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void transactionFailure(@Observes TransactionFailure event) {
        ...
        try (KafkaConnection connection = factory.createConnection()) {
            connection.send(new ProducerRecord<>("transactionFailures", playerId, toJsonString(new FailureMessage(event.getTransactionMessage().getId(), event.getFailure()))));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
